 ...
 */
export function confirmTrade(summary, assumeYes) {
    printSummary(summary);
    const automationOptIn = process.env[AUTOMATION_ENV] === "1";
    if (assumeYes) {
        if (automationOptIn) {
            console.error(`[gmgn-cli] Proceeding non-interactively (--yes + ${AUTOMATION_ENV}=1).`);
            return;
        }
        // --yes alone is deliberately NOT enough: an injected agent can pass it.
        abort(`--yes was supplied but ${AUTOMATION_ENV}=1 is not set in the environment. ` +
    ...
    if (answer == null) {
        abort(`No interactive terminal available to confirm this ${summary.action.toLowerCase()}. ` +
