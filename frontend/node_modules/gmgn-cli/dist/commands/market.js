...
function parseDuration(value) {
    if (/^\d+(\.\d+)?[sm]$/.test(value))
        return value;
    if (/^\d+(\.\d+)?$/.test(value)) {
        console.warn(`[gmgn-cli] Warning: no unit specified for duration "${value}" — treating as minutes (${value}m). Use a suffix to be explicit: ${value}s for seconds or ${value}m for minutes.`);
        return `${value}m`;
    }
    console.error(`[gmgn-cli] Invalid duration "${value}". Use seconds (e.g. 30s) or minutes (e.g. 0.5m / 1m / 5m).`);
    process.exit(1);
}
...
function parseSearchBool(value, flag) {
...
export function registerMarketCommands(program) {
    ...
    market
        ...
        .requiredOption("--resolution <resolution>", "Candlestick resolution: 1s (Pro only) / 30s / 1m / 5m / 15m / 1h / 4h / 1d")
        ...
        const data = await client
            .getTokenKline(opts.chain, opts.address, opts.resolution, opts.from != null ? opts.from * 1000 : undefined, opts.to != null ? opts.to * 1000 : undefined)
            .catch(exitOnError);
    ...
    trendingCmd.action(async (opts) => {
        ...
        if (opts.orderBy)
            extra["order_by"] = opts.orderBy;
        if (opts.direction)
            extra["direction"] = opts.direction;
        if (opts.filter?.length)
            extra["filters"] = opts.filter;
        if (opts.platform?.length)
            extra["platforms"] = opts.platform;
    ...
    trenchesCmd.action(async (opts) => {
        ...
        if (opts.filterPreset != null) {
            const preset = TRENCHES_FILTER_PRESETS[opts.filterPreset];
            if (!preset) {
        ...
        const result = opts.sortBy
            ? sortTrenchesResult(data, opts.sortBy, opts.direction ?? "")
            : data;
    ...
    market
        ...
        .option("--signal-type <n...>", "Signal type(s), repeatable: 1–21 (default: all types)", (v, acc) => { acc.push(parseInt(v, 10)); return acc; }, [])
        ...
        .action(async (opts) => {
        validateChain(opts["chain"]);
        if (!["sol", "bsc", "robinhood", "arc", "stable"].includes(opts["chain"])) {
        ...
        if (opts["groups"] != null) {
            ...
            const group = {};
            const signalType = opts["signalType"];
            if (signalType?.length)
                group.signal_type = signalType;
            if (opts["mcMin"] != null)
                group.mc_min = opts["mcMin"];
            if (opts["mcMax"] != null)
                group.mc_max = opts["mcMax"];
            if (opts["triggerMcMin"] != null)
                group.trigger_mc_min = opts["triggerMcMin"];
            if (opts["triggerMcMax"] != null)
                group.trigger_mc_max = opts["triggerMcMax"];
            if (opts["totalFeeMin"] != null)
                group.total_fee_min = opts["totalFeeMin"];
            if (opts["totalFeeMax"] != null)
                group.total_fee_max = opts["totalFeeMax"];
            if (opts["minCreateOrOpenTs"] != null)
                group.min_create_or_open_ts = opts["minCreateOrOpenTs"];
            if (opts["maxCreateOrOpenTs"] != null)
                group.max_create_or_open_ts = opts["maxCreateOrOpenTs"];
            groups = [group];
        }
        const client = new OpenApiClient(getConfig());
        const data = await client.getTokenSignalV2(opts["chain"], groups).catch(exitOnError);
        printResult(data, opts["raw"]);
    });
    const hotSearchesCmd = market
        ...
        .option("--chain <chain...>", "Chain(s), repeatable: sol / bsc / base / eth / arbitrum / hyperevm / robinhood / arc / stable (default: all default chains)")
    ...
    hotSearchesCmd.action(async (opts) => {
        ...
        if (opts.params != null) {
            ...
            const optsMap = opts;
            const chains = opts.chain?.length ? opts.chain : [];
            params = chains.map((chain) => {
                const param = { label: "hot-search", chain, interval };
                if (opts.filter?.length)
                    param.filters = opts.filter;
                if (opts.limit != null)
    ...
    market
        ...
        if (opts.chain) {
            ...
            if (opts.chain === "all") {
                console.warn('[gmgn-cli] Note: --chain all is not accepted upstream (it returns no results). Searching all chains by omitting the chain filter instead.');
            }
            else {
        ...
        if (opts.launchpadPlatform?.length) {
            const platforms = opts.launchpadPlatform;
        ...
        if (opts.isOg != null)
            extra["is_og"] = parseSearchBool(String(opts.isOg), "--is-og");
        if (opts.isLaunched != null)
            extra["is_launched"] = parseSearchBool(String(opts.isLaunched), "--is-launched");
...
function apiFieldToCliKey(apiField) {
    return apiField
        .replace(/_/g, '-')
